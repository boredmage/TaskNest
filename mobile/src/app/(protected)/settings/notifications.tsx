...
const NotificationRow = ({
  ...
  return (
    <View>
      ...
      <Text className="text-hint mt-1 px-1 text-sm">{description}</Text>
    </View>
  );
};
...
const Notifications = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) console.error("[NOTIF PREFS] load error:", error);
      }
    })();
    return () => {
      cancelled = true;
  ...
  const persist = async (next: NotificationSettings) => {
    try {
      await api.put("/notification-preferences", { preferences: next });
    } catch (error) {
